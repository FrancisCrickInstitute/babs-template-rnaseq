defaults <- list(
  ...
  metadata    = "inst/extdata/metadata.xlsx",    ## Path to  spreadsheet containing covariates
  file_col    = c("filename", "sample", "sample_ID", "Sample.limsid"),    ## Column in that spreadsheet that identifies the basename of count file for each sample
  name_col    = c("name", "label", "sample", "filename"),    ## Column in that spreadsheet that identifies the basename of count file for each sample
  counts      = "../results/star_rsem",      ## directory that contains the quantified gene counts
  org_package = "", ## NULL will derive org.*.eg.db from nfcore info, otherwise specifcy string identifying annotation package of relevant species.
  spec_prefix = ""
)
