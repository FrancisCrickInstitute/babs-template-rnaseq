# The following are system paths. Please set them to agree with your
# local system. Watch out for trailing spaces, makefiles are very
# 'literal' so a trailing spaceeven  between the value and
# the comment character '#' can get included value

# where .sif's are stored:
SINGULARITY_ROOT=.

# local renv cache:
RENV_PATHS_ROOT=.

# your chosen prefix (e.g.'rocker') to keep the cache somewhat isolated:
RENV_PATHS_PREFIX=

# working space for large disposable files:
SCRATCH_DIR=.

# Nextflow cache:
NXF_SINGULARITY_CACHEDIR=.

