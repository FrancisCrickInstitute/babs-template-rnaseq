...
In @fig-summary-p-1 we present some diagnostic plots of the observed
...
for (dataset in names(summaries)) {
  for (model in names(pval_frame[[dataset]])) {
    pl <- ggplot(pval_frame[[dataset]][[model]], aes(x=pvalue, fill=baseMean<1)) +
      ...
      theme_bw()
    do_plot(pl,
            label=fig_label("summary-p"),
            caption=paste0("p-Diagnostic for", dataset, model)
            )
    cat("\n\n---\n\n")
  }
}
