...
`dplyr::mutate` clause, but with no need for the initial data object:
...
- see the [this section]{#cascade} for a list of behaviours.
...
```{r, eval = FALSE}
    all = sample_set(
      ...
      models = list(
	    ...
	    aes(x=time, colour=treatment) ~ . - batch
	  )
	  ...
```
...
comparisons` hierarchy.  We list the expected behaviour of those:
 ...
 - `profile_plots` can be set at the `specification` and/or `sample_set` level, and will be added to thos of any model-specific plot profiles
