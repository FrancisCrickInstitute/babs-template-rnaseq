...
There's also a `differential_subset` field that takes the same form.
...
restriction of the test as `differential_subset = condition ==
...
samples (larger number of samples/columns).
...
comparisons` hierarchy.  We list the expected behaviour of those:
 ...
 - `subset` can be set at the top `specification` level, and will get boolean-`AND`ed with  sample-set specific subsets.
